...
function load_wilson_flow(wf_no)
    output = read(wf_file_location(wf_no), String)

    raw_df = _convert_outformat_to_dataframe(output)
    
    data = DataFrame([Int64[], Float64[], Float64[], Float64[], Float64[], Float64[], Float64[], Float64[], Float64[]], [:conf_no, :time, :plaquette, :t, :E, :t²E, :Esym, :t²Esym, :TC])
...
function extract_global_metadata(first_run::DataFrame)
    ranlux = only(match(r"^RLXD (.+)", only(first_run[first_run.name.=="SETUP_RANDOM", :output])).captures)
